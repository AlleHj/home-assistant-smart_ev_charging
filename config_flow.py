# File version: 2025-05-29 0.1.25
...
def _build_common_schema(
    ...
    defined_fields_with_selectors[CONF_SOLAR_PRODUCTION_SENSOR] = (
        _get_current_value(CONF_SOLAR_PRODUCTION_SENSOR),
        EntitySelector(
            EntitySelectorConfig(
                domain="sensor", device_class=SensorDeviceClass.POWER, multiple=False
            )
    ...
    defined_fields_with_selectors[CONF_SOLAR_SCHEDULE_ENTITY] = (
        _get_current_value(CONF_SOLAR_SCHEDULE_ENTITY),
        EntitySelector(EntitySelectorConfig(domain="schedule", multiple=False)),
    )
    defined_fields_with_selectors[CONF_CHARGER_MAX_CURRENT_LIMIT_SENSOR] = (
        _get_current_value(CONF_CHARGER_MAX_CURRENT_LIMIT_SENSOR),
        EntitySelector(EntitySelectorConfig(domain="sensor", multiple=False)),
    )
    defined_fields_with_selectors[CONF_EV_POWER_SENSOR] = (
        _get_current_value(CONF_EV_POWER_SENSOR),
        EntitySelector(
            EntitySelectorConfig(
                domain="sensor", device_class=SensorDeviceClass.POWER, multiple=False
            )
    ...
    defined_fields_with_selectors[CONF_EV_SOC_SENSOR] = (
        _get_current_value(CONF_EV_SOC_SENSOR),
        EntitySelector(
            EntitySelectorConfig(
    ...
    defined_fields_with_selectors[CONF_TARGET_SOC_LIMIT] = (
        _get_current_value(CONF_TARGET_SOC_LIMIT),
        NumberSelector(
            NumberSelectorConfig(
                min=0,
                max=100,
    ...
    defined_fields_with_selectors[CONF_SCAN_INTERVAL] = (
        _get_current_value(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL_SECONDS),
        NumberSelector(
            NumberSelectorConfig(
                min=10,
                max=3600,
    ...
    defined_fields_with_selectors[CONF_DEBUG_LOGGING] = (
        _get_current_value(CONF_DEBUG_LOGGING, False),
    ...
    for conf_key, (
        ...
        if is_options_flow:
            final_schema_dict[
                vol.Optional(conf_key, default=schema_validation_default)
            ] = current_selector_instance
        else:
            if is_initial_setup_display:
                if conf_key in REQUIRED_CONF_SETUP_KEYS:
                    ...
                    default_to_use_for_setup = (
                        vol.UNDEFINED
                        if conf_key == CONF_TARGET_SOC_LIMIT
                        and actual_current_val is None
                        else schema_validation_default
                    )
                    final_schema_dict[
                        vol.Optional(conf_key, default=default_to_use_for_setup)
                    ] = current_selector_instance
            else:
                if conf_key in REQUIRED_CONF_SETUP_KEYS:
                    final_schema_dict[
                        vol.Required(conf_key, default=ui_display_default)
                    ] = current_selector_instance
                else:
                    final_schema_dict[
                        vol.Optional(conf_key, default=ui_display_default)
                    ] = current_selector_instance
...
class SmartEVChargingOptionsFlowHandler(OptionsFlow):
    ...
    def __init__(self, config_entry: ConfigEntry) -> None:
        ...
        self.config_entry tillhandahålls av basklassen OptionsFlow.
        """
        # self.config_entry = config_entry # Denna rad är överflödig och orsakar deprecation warning.
        pass
    ...
    async def async_step_init(
        ...
        errors: Dict[str, str] = {}
        # self.config_entry är tillgänglig från OptionsFlow basklassen.
        current_settings = {**self.config_entry.data, **self.config_entry.options}

        if user_input is not None:
            # Loggraden för user_input har tagits bort igen då den har tjänat sitt syfte för diagnos.
            # Diagnosen pekar på att user_input från frontend inte korrekt reflekterar
            # att ett entitetsfält har rensats, utan skickar det gamla värdet.
            _LOGGER.debug(
                "OptionsFlow: Mottaget user_input (för felsökning av sparproblem): %s",
                user_input,
            )  # Behålls tills vidare på begäran.
...
class SmartEVChargingConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    def async_get_options_flow(
        ...
        # config_entry skickas automatiskt till OptionsFlowHandler.__init__ av Home Assistant.
